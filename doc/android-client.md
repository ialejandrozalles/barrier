...
## Menú profesional de atajos

La app incluye un menú de opciones para configurar atajos globales sin editar código.

Ruta:

1. Abrir la app Android.
2. Pulsar el botón de menú en la barra superior.
3. Entrar en **Atajos**.
...
Cada acción permite seleccionar tecla base y modificadores (`Ctrl`, `Alt`, `Shift`, `Meta`, `Super`).

Valores predeterminados (compatibles con el comportamiento anterior):

- `Esc` -> Atrás
- `Ctrl+Esc` -> Inicio
- `Alt+Tab` -> Recientes
- `Ctrl+N` -> Notificaciones
- `Ctrl+Q` -> Ajustes rápidos
- `Ctrl+P` -> Diálogo de apagado
